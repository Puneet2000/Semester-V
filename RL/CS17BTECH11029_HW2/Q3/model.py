...
class SparseAE (Model):
	def __init__(self,nodes,p=0.4,lam=0.5):
		super(SparseAE,self).__init__(nodes=nodes)
		self.p = p
		self.lam =  lam
	...
	def backward(self,x,y):
		out = self.forward(x)
		prob = np.mean(self.outputs[self.bt_index],axis=1)
		loss = (1./(2)) * np.sum((out - y) ** 2)
		sp_loss = self.lam*np.sum(self.p*np.log(self.p/(prob)) + (1-self.p)*np.log((1-self.p)/(1-prob + 1e-10)))*x.shape[1]
		self.errors = [0]*(self.len-1)
		self.errors[-1] = (self.outputs[-1]-y)*self.d_activation(self.outputs[-1])
		for i in range(len(self.errors) - 2, -1, -1):
			self.errors[i] = (np.dot(np.transpose(self.weights[i+1]),self.errors[i+1]))*self.d_activation(self.outputs[i+1]) 
			if i == self.bt_index-1:
				kl_grad = self.lam*(-self.p/(prob) + (1-self.p)/(1-prob+1e-10))
				self.errors[i] += np.multiply(np.transpose(np.matrix(kl_grad)),self.d_activation(self.outputs[i+1]))
		return loss,sp_loss
