...
from model import *
import argparse
parser = argparse.ArgumentParser(description='sparse AE')
...
print(args)
def lr(epoch): # learning rate scheduler
...
epochs=args.epochs
model = SparseAE(args.layers,p=args.sparsity)
...
for i in range(epochs):
	...
	for j in range(0,len(x_train),batch_size):  # sample batch
		...
		loss,sp_loss = model.update(batch,batch,lr(i))
		avg_loss += loss
		avg_sp_loss += sp_loss
	...
	print('Epoch {}, Loss : {:3f}, Sparse Loss {:3f}'.format(i,avg_loss/len(x_train),avg_sp_loss/len(x_train)))
